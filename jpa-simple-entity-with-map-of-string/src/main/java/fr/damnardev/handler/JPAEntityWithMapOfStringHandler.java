...
@Slf4j
public class JPAEntityWithMapOfStringHandler implements ApplicationRunner {
	...
	public void run(ApplicationArguments args) {
		...
		var dbUser = DbUser.builder().name("Foo").datas(emails).build();
		...
		var updated = loaded.map((user) -> {
			user.getDatas().put("email", "foo@bar.com");
